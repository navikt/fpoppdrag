...
class SystemPropertiesHelper {
    ...
    private static final List<String> SECRET_NAME_PARTS = Arrays.asList("passord", "password", "passwd", "key");
    private static final String FILTERED_VALUE = "*****";
    private static final Pattern SECRET_VERDI_REGEX = Pattern.compile("([^= ]*?(?:passord|password|passwd|key)[^=]*\\s*=\\s*)[^ ]*", Pattern.CASE_INSENSITIVE);
    ...
    static void filter(Map<String, String> map) {
        for (Map.Entry<String, String> entry : map.entrySet()) {
            ...
            if (containsSecretNamePart(entry.getValue())) {
                ...
                String filtered = matcher.replaceAll(FILTER_PATTERN);
