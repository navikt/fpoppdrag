package no.nav.foreldrepenger.oppdrag.konfig;
...
import no.nav.foreldrepenger.oppdrag.kodeverk.KonfigVerdiGruppe;
...
public class DatabaseKonfigVerdiProvider implements KonfigVerdiProvider {
    ...
    private Optional<KonfigVerdiEntitet> getVerdiInternal(String key) {
        return konfigVerdiRepository.finnVerdiFor(KonfigVerdiGruppe.INGEN_GRUPPE, key, FPDateUtil.iDag());
    }
    ...
    public <V> List<V> getVerdier(String key, Converter<V> converter) {
        List<KonfigVerdiEntitet> list = konfigVerdiRepository.finnVerdierFor(KonfigVerdiGruppe.forGruppe(key), FPDateUtil.iDag());
