    ...
    private void verifisertTilkjentYtelseAndel(List<TilkjentYtelseAndel> tyAndeler) {
        for (TilkjentYtelseAndel andel : tyAndeler) {
            ...
            if (gjelderPeriode1) {
                ...
                assertThat(andel.getTilkjentYtelseFeriepenger()).isNotPresent();
            } else {
                if (andel.getUtbetalesTilBruker()) {
                    ...
                    assertThat(andel.getTilkjentYtelseFeriepenger()).isPresent();
                    assertThat(andel.getTilkjentYtelseFeriepenger().get().getOpptjeningsår()).isEqualTo(2018);
                    assertThat(andel.getTilkjentYtelseFeriepenger().get().getÅrsbeløp()).isEqualTo(187);
                }
            }
        }
    }
